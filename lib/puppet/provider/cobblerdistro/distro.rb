...
Puppet::Type.type(:cobblerdistro).provide(:distro) do
  ...
  def create
    ...
    if @resource[:path]
      ...
      # get ISO image
      wget(@resource[:isolink],'--continue','--directory-prefix=' + @resource[:tmpdir]).strip unless File.exists?(@resource[:tmpdir] + '/' + @resource[:isolink])

      # get ISO path
      isopath = @resource[:tmpdir] + '/' + @resource[:isolink].sub(/^.*\/(.*).iso/, '\1')
